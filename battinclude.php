			...
			<div class="col-11 formcontainer" id="">
			    ...
			    <form class="incform" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post"> 
				    <div>
				        ...
				        <input type="text" name="serial" value="" placeholder="Serial" class="input1" required maxlength="15">
				        &nbsp;&nbsp;
				        <input type="text" name="voltage" value="" placeholder="Voltage" class="input1" required maxlength="5">
				        &nbsp;&nbsp;
	                    <input type="text" name="cells" value="" placeholder="Cells" class="input1" maxlength="4" required>
	                    &nbsp;&nbsp;
	                    <input type="text" name="capacity" value="" placeholder="Capacity" class="input1" required maxlength="5">
	                    <p>First Use</p>
	                    <input type="date" name="firstuse" value="" class="input1">
	                    <br>
	                    <p>Purchase Date</p>
	                    <input type="date" name="purchasedate" value="" class="input1" required>
	                    &nbsp;
	                    <label class="label1">Battery Status</label>&nbsp;
				        <select name="status" class="input1">
				            <option value"0" class="tabb"></option>
                                    <?php
				                    $sqlsent="SELECT * FROM status;";
				    				...
				    				if ($result2check > 0)
	                			    {
	                			    	while ($row = mysqli_fetch_assoc($result2))
	                			        {
	                			           ...
	                			           echo "<option class='tabd ".$row['name']."' value=".$row['status_id'].">".$row['name']."</option>"; 
	                			        } 
	                			    }
	                			    ?>
	                	</select>		    
