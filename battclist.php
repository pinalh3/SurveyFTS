		...
		<div class="col-10" id="form1">
			...
			<div class="col-11 formcontainer" id="">
				...
				<?php
                    ...
                    $sqlsent="SELECT * FROM charge INNER JOIN crew ON charge.crew_id= crew.crew_id  where batteries_id=$battery;";
                    $result1=mysqli_query($conn, $sqlsent);
			        $result1check = mysqli_num_rows($result1);
                    
                    	if ($result1check > 0)
	                			    {
	                			        echo "<p>Charge Cycles ".$result1check."</p><br>";
	                			        echo "<table class='query'>";
	                			        echo "<tr class='tabheader'><th>Item</th><th>Date of Charge</th><th>V Bef Charge</th><th>V Aft Charge</th><th>Charged By</th><th>s</th></tr>";
			                while ($row = mysqli_fetch_assoc($result1))
			                {
			        
			                echo "<tr class='tabd'><td>".$row['chargecycle_id']."</td><td>".$row['date_charge']."</td><td>".$row['bvolt_before']."V</td><td>".$row['bvolt_after']."V</td><td>".$row['first_name']." ".$row['first_surname']."</td><td><form action='batteriescmod.php' method='post'><input type='text'name='view1' value='".$row['chargecycle_id']."' class='hi'><input type='submit'name=submit value='view'></form></td></tr>"; 
			                }
			                echo "</table>";
			                }
			                else
