    ...
    if (isset($_POST['submit'])) { 
        ...
        $uav2=$_POST['uav'];
			...
			$result2check = mysqli_num_rows($result2); 
                ...
                $field26= mysqli_real_escape_string($conn,$releasebefore);
                $field27= mysqli_real_escape_string($conn,$releaseafter);
                ...
                $query1 = "INSERT INTO flightslog (flights_id, uav_id, batteries_id, payload_id, fdate, start_time, end_time, wconditions_id, opeobj_id, antenna_id, windspeed, flight_temperature, time_flight, battery_usage, maxheight, comments, operation_area_id)
                ...
                $sent1= mysqli_query($conn,$query1);
                    ...
                    else
                    {
                        ...
                        {
                            $releasecount="0";
                            echo "release after activo";
                        }
                        elseif (strcasecmp($releasebefore,$d) == 0)
                        {
                            $releasecount="1";
                            echo "release before activo";
                            echo $releasebefore;
                            echo $releaseafter;
                        ...
                        else
                        {
    			            ...
    			            echo "la verificacion rel $resultcheckrel";
                			 if ($resultcheckrel > 0)
                            {
                                while ($row = mysqli_fetch_assoc($resultrel))
                                {
                                  $count=$row['release_count'];  
                                  echo "Esta es la cuenta que lleva $count";
                                  $releasecount=++$count;
                                  echo "Asi se debe almacenar $releasecount";
                                }
                                if($count>=5)
                                {
                        ...
                        $sent9= mysqli_query($conn,$query9); 
                            ...
                            {
                                $errorp="flight log ".$flights_id." added";
